...
# .font100[Caracteristicas]

.font105[.line-space1-5[
La base de datos fue extraida de una investigación que consistía en una correlación entre la `Autoeficacia Académica (AA)` y el `Burnout (B)` originado por el aprendizaje a distancia en estudiantes de enfermería ]]

.pull-right[
`Muestra`          = 69 estudiantes
`Sexo`             = Hombre, Mujer
`Edad`             = 17-23 años

]
.pull-right[
`Convivencia`      = Solo, Con familia
`Escuela`          = Science, Social Science, Health Vocation, Non-health vocat
`Escala de AA`     = ase1:ase31
]

.pull-left[

`Escala de B`        = bur1:bur24
`Puntaje total (AA)` = de 44 a 114 
`Puntaje total (B)`  = de 10 a 125
`Puntaje categorizado (AA)` = bajo, moderado, alto
`Puntaje categorizado (B)`  = leve, moderado, severo
]
...
.pull-left[
**geom_bar**
La imagen puede apreciarse de forma más detallada en el siguiente link (https://ars.els-cdn.com/content/
]
